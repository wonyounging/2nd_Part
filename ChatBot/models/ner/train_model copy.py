...
# 말뭉치 데이터에서 단어와ㅏ BIO 태그만 불러와 학습용 데이터셋 생성
sentences, tags = [], []
for t in corpus:
    tagged_sentence = []
    sentence, bio_tag = [], []
...
# 토크나이저 정의
tag_tokenizer = preprocessing.text.Tokenizer(lower=False)   # 태그 정보는 lower=False 소문자로 변환 x
...
model.save('./ChatBot/models/ner/ner_model_03_up_10.h5')
...
def sequences_to_tag(sequences):    # 예측값을 index_to_ner를 사용하여 태깅 정보로 변경하는 함수
    ...
    for sequence in sequences:      # 전체 시퀀스로부터 시퀀스를 하나씩 꺼냄
        temp = []
        for pred in sequence:       # 시퀀스로부터 예측값을 하내씩 꺼냄
