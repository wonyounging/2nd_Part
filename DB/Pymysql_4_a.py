...
class GoodsRead :
    ...
    def goodsDelete(self):      # 삭제기능
        try :
            ...
            del_code = int(input(f'삭제할 {self.read_sel} 입력 : '))
            sql = f"select * from goods where code = {del_code}"
            cursor.execute(sql)
            rows = cursor.fetchall()

            if rows:
                print('삭제 성공')
                sql = f"delete from goods where code = {del_code}"
                cursor.execute(sql)
                conn.commit()
            else:
                print('삭제 실패했습니다.')
