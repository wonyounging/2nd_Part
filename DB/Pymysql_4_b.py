...
class GoodsRead :
    ...
    def goodsUpdate(self):
        try:
            ...
            up_code = int(input("수정할 코드 입력 : "))
            # 수정 전 레코드 출력
            sql = f"select * from goods where code = {up_code}"
            cursor.execute(sql)
            ...
            if rows:
                ...
                check = input("수정하시겠습니까?(Y/N) : ")  # 수정 실행 여부 확인
                check = check.upper()
                # print(check)
                if check == "Y":
                    # print('예')
                    os.system('cls')
                    print("<<<수정할 내용을 입력하세요>>>")
                    up_name = input("수정할 상품명 입력 : ")
                    up_su = int(input("수정할 수량 입력 : "))
                    up_dan = int(input("수정할 단가 입력 : "))
                    sql = f"update goods set code = {up_code}, name = '{up_name}', su = {up_su}, dan = {up_dan} where code ={up_code}"
